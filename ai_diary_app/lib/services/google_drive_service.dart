...
class GoogleDriveService {
  ...
  static Future<bool> uploadBackup({required bool isPremium}) async {
    try {
      if (kDebugMode) print('=== Google Drive 백업 시작 ===');
      final googleSignIn = GoogleSignIn(scopes: [drive.DriveApi.driveFileScope]);

      if (kDebugMode) print('Google Sign In 확인 중...');
      final account = await googleSignIn.signInSilently();

      if (account == null) {
        if (kDebugMode) print('❌ Google 로그인이 필요합니다');
        return false;
      }

      if (kDebugMode) print('✅ Google 계정 확인됨: ${account.email}');

      if (kDebugMode) print('인증 클라이언트 가져오는 중...');
      final httpClient = (await googleSignIn.authenticatedClient())!;
      final driveApi = drive.DriveApi(httpClient);
      if (kDebugMode) print('✅ Drive API 클라이언트 생성 완료');

      // 백업 폴더 가져오기 또는 생성
      if (kDebugMode) print('백업 폴더 확인 중...');
      final folderId = await _getOrCreateFolder(driveApi);
      if (kDebugMode) print('✅ 폴더 ID: $folderId');

      // DatabaseService의 exportToJson 사용하여 표준 포맷으로 백업
      if (kDebugMode) print('백업 데이터 생성 중...');
      final jsonString = await DatabaseService.exportToJson(isPremium: isPremium);
      final bytes = utf8.encode(jsonString);
      if (kDebugMode) print('✅ JSON 크기: ${bytes.length} bytes');

      // 기존 파일 검색
      if (kDebugMode) print('기존 백업 파일 검색 중...');
      ...
      if (existingFiles.files != null && existingFiles.files!.isNotEmpty) {
        if (kDebugMode) print('기존 백업 파일 삭제 중...');
        await driveApi.files.delete(existingFiles.files!.first.id!);
        if (kDebugMode) print('✅ 기존 파일 삭제 완료');
      } else {
        if (kDebugMode) print('기존 백업 파일 없음');
      }

      // 새 파일 업로드
      if (kDebugMode) print('새 백업 파일 생성 중...');
      ...
      if (kDebugMode) print('파일 업로드 중...');
      await driveApi.files.create(file, uploadMedia: media);

      if (kDebugMode) print('✅ Google Drive 백업 성공!');
      return true;
    } catch (e) {
      if (kDebugMode) print('❌ Google Drive 백업 실패: $e');
      if (kDebugMode) print('에러 타입: ${e.runtimeType}');
      if (kDebugMode) print('스택 트레이스: ${StackTrace.current}');
      return false;
    }
  }
  ...
  static Future<bool> hasBackup() async {
    try {
      ...
      if (kDebugMode) print('Google Drive 백업 확인 실패: $e');
      return false;
    }
  }
  ...
  static Future<int> downloadAndRestoreBackup() async {
    try {
      ...
      if (folderList.files == null || folderList.files!.isEmpty) {
        if (kDebugMode) print('백업 폴더를 찾을 수 없습니다');
        return 0;
      }
      ...
      if (fileList.files == null || fileList.files!.isEmpty) {
        if (kDebugMode) print('백업 파일을 찾을 수 없습니다');
        return 0;
      }
      ...
      if (kDebugMode) print('Google Drive 복원 성공: $restoredCount개');
      return restoredCount;
    } catch (e) {
      if (kDebugMode) print('Google Drive 복원 실패: $e');
      return 0;
    }
  }
}
